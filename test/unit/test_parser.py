...
test_parser_list = [
    x.split("/")[-1] for x in glob("{}/test_parser/list/*".format(BASE_PATH))
]
test_parser_leaf = [
    x.split("/")[-1] for x in glob("{}/test_parser/leaf/*".format(BASE_PATH))
]
test_parser_container = [
    x.split("/")[-1] for x in glob("{}/test_parser/container/*".format(BASE_PATH))
]
...
class Tests(object):
    ...
    def test_parser_list(self, case):
        with open(
            "{}/test_parser/list/{}/mocked.txt".format(BASE_PATH, case), "r"
        ) as f:
            mocked = f.read()
        with open(
            "{}/test_parser/list/{}/example.yaml".format(BASE_PATH, case), "r"
        ) as f:
        ...
        bookmarks = {
            "root_{}".format(example["processor"]["root_name"]): mocked,
            "parent": parent,
        }
        ...
        for case, d in enumerate(example["data"]):
            ...
            for k, b, e in parser.parse_list(attribute, example["rule"], bookmarks):
                ...
                assert b == example["expected"][case][i]["block"], "\n{}".format(
                    yaml.safe_dump(to_dict(b), default_flow_style=False)
                )
                assert e == example["expected"][case][i]["extra_vars"], "\n{}".format(
                    yaml.safe_dump(to_dict(e), default_flow_style=False)
                )
                i += 1
    ...
    def test_parser_leaf(self, case):
        with open(
            "{}/test_parser/leaf/{}/mocked.txt".format(BASE_PATH, case), "r"
        ) as f:
            mocked = f.read()
        with open(
            "{}/test_parser/leaf/{}/example.yaml".format(BASE_PATH, case), "r"
        ) as f:
            example = yaml.load(f.read())
        ...
        for case, d in enumerate(example["data"]):
            ...
            #  parser.extra_vars = d["extra_vars"]
            i = 0
            for k, b, e in parser.parse_list(
                attribute, example["parent_rule"], bookmarks
            ):
                bookmarks["parent"] = b
    ...
    def test_parser_container(self, case):
        with open(
            ...
            mocked = f.read()
        with open(
            "{}/test_parser/container/{}/example.yaml".format(BASE_PATH, case), "r"
        ) as f:
            example = yaml.load(f.read())
        ...
        for case, d in enumerate(example["data"]):
            ...
            #  parser.keys = d["keys"]
            #  parser.extra_vars = d["extra_vars"]
            d, extra_vars = parser.parse_container(
                "aintmatter", example["rule"], bookmarks
            )
